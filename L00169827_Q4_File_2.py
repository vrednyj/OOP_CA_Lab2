...
def create_dictionary():
    ...
    try:
        ...
        csvfile: TextIO = open(file_name, 'r')
...
def port_scan() -> None:
    ...
    try:
        remote_server = input(
            "Enter a remote host to scan: ")  # Will use "try:" to avoid using ip_validator.
        # This allows to scan ports on remote and local hosts.
    ...
    try:
        for port in range(18, 24):
            ...
            if result == 0:
                print("Port {0}: \tService name: {1:^20} Status: Opened".format(port, str(dict_of_port_name.get(
                    str(port))).upper()))  # Using .get method to avoid exception.
            else:
                print("Port {0}: \tService name: {1:^20} Status: Closed".format(port, str(dict_of_port_name.get(
                    str(port))).upper()))
            sock.close()
